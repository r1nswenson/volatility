...
void help(TCHAR *ExeName) {
  ...
  Log(L"  -l    Load the driver and exit.\n"
      ...
      L"  -m [output_json_file] Get loaded modules\n"
      L"\n");
  ...
  for (i = 1; i < argc; i++) {
    if (argv[i][0] == '-' && argv[i][1] != 0) {
      switch (argv[i][1]) {
        ...
        case  'm': {
            Log(TEXT("Getting list of loaded modules.\n"));
            if (argc != 3)
              goto error;

            ModulesLoadedClass modulesLoaded;
            if (modulesLoaded.Initialize() && modulesLoaded.ExportTo(argv[i + 1]))
              return 0;
            else
              return -1;
          }
