...
def load_as(config, astype = 'virtual', **kwargs):
    ...
    with UpdateCounterForScope('load_as'):
        base_as = None
        ...
        while found:
            ...
            found = False
            #print registry.get_plugin_classes(addrspace.BaseAddressSpace)
            for cls in sorted(registry.get_plugin_classes(addrspace.BaseAddressSpace).values(),
                ...
                try:
                    ...
                    debug.debug("Succeeded instantiating {0} order {1}".format(base_as, cls.order))
                    found = True
                    break
