...
if "%BUILD_CREDS%" == "true" ( 
  echo Signing via request to signing server
  @echo on
  "%~dp0..\..\..\target\dependency\tools\CM\Signing\SigningTool.exe" winpmem.sys -t
  @echo off
) else if exist "c:\Protected\After_10-10-10_MSCV-VSClass3.cer" (
  echo Signing using official certificate
  @echo on
  "%~dp0SignTool.exe" sign /ac c:\Protected\After_10-10-10_MSCV-VSClass3.cer /n AccessData /t http://timestamp.verisign.com/scripts/timestamp.dll winpmem.sys
  @echo off
) else (
