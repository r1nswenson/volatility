...
def getConfigValue(dict,key,log = True):

    if(key in dict and dict[key] == 'true'):
        return True
    if dict.has_key(key) == False and log :
        logging.error("{} key was not found in the configuration.".format(key))

    return False

def _utf8_encode(stringdata):
    encoding="utf8"
    try:
        # Allow the object to have a __unicode__ method.
        return str(stringdata).encode(encoding, "replace")
    except UnicodeError:
        return ""
...
def SmartUnicode(string, encoding="utf-8"):
    """Forces the string into a unicode object."""
    try:
        # Allow the object to have a __unicode__ method.
        return unicode(string).encode(encoding, "replace")
    except UnicodeError:
        return str(string).encode(encoding, "replace")

def convert_time(time):
    """
    Converts emtpy time to '0000-00-00 00:00:00'.
    :param time: a timestamp
    :return: a utf-8 encoded time string set to 0's when empty.
    """
    if time.value:
        return SmartUnicode(time)
    else:
