...
VOID IoUnload(IN PDRIVER_OBJECT DriverObject) {
  ...
  WinDbgPrint(ATAG("winpem IoUnload called\n"));
...
static NTSTATUS wddCreate(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp) {
  ...
  WinDbgPrint(ATAG("Handle Opened\n"));
...
static NTSTATUS wddClose(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp) {
  ...
  WinDbgPrint(ATAG("Handle Closed\n"));
  ...
  switch (IoControlCode) {
    ...
    case IOCTL_GET_INFO: {
        ...
        if (ext->mode == ACQUISITION_MODE_PTE_MMAP_WITH_PCI_PROBE) {
          WinDbgPrint(ATAG("PCI_AddMemoryRanges\n"));
          status = PCI_AddMemoryRanges(info, OutputLen);
        }
        else {
          WinDbgPrint(ATAG("AddMemoryRanges\n"));
    ...
    case IOCTL_VIRTUAL_ADDRESS_IS_VALID: {
        //WinDbgPrint("Page-in virtual address\n");
        if (InputLen >= sizeof(VirtualAddressInfo) && OutputLen >= sizeof(BOOLEAN)) {
          VirtualAddressInfo *virtualAddressInfo = (VirtualAddressInfo *)IoBuffer;

          BOOLEAN *addressIsValid = (BOOLEAN *)IoBuffer;

          *addressIsValid = VirtualAddressIsValid(virtualAddressInfo);
...
NTSTATUS DriverEntry(IN PDRIVER_OBJECT DriverObject,
  ...
  WinDbgPrint(ATAG("WinPMEM - " PMEM_VERSION " - Physical memory acquisition\n"));

#if PMEM_WRITE_ENABLED == 1
  WinDbgPrint("WinPMEM write support available!");
#endif

  WinDbgPrint(ATAG("Copyright (c) 2014, Michael Cohen <scudette@gmail.com>\n"));
