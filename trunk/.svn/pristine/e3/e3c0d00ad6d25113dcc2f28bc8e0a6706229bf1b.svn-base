...
NTSTATUS PmemRead(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp) {
  ...
  switch (extension->mode) {
    ...
    case ACQUISITION_MODE_PTE_MMAP:
      ...
      //if (BufOffset.QuadPart == 0x00086000)
      //  WinDbgPrint(ATAG("ACQUISITION_MODE_PTE_MMAP BufOffset::%08I64x Skipped\n"), BufOffset.QuadPart);
      status = DeviceRead(extension, BufOffset, Buf, BufLen, &total_read, PTEMmapPartialRead);
      break;

    default:

      //WinDbgPrint(ATAG("Acquisition mode %u not supported.\n"), extension->mode);
      status = STATUS_NOT_IMPLEMENTED;
      BufLen = 0;
  }

  //if (printCount++ % 1024 * 1024 * 8 == 0)
  //  WinDbgPrint(ATAG("MODE:%d BufOffset::%0I64x BufLen:0x%04lx total_read:0x%04lx status:0x%04lx\n"), extension->mode, BufOffset.QuadPart, BufLen, total_read, status);
